...
INT_PTR CALLBACK DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		hNickName = GetDlgItem(hDlg, IDC_NICKNAMETEXT);
	...
	case WM_COMMAND:
		...
		case IDC_SENDMSG:
			...
			// 새로운 채팅 메시지를 얻고 쓰기 완료를 알림 //nickname+ ': ' + g_chatmsg.msg
		...
		case IDC_NICKNAMEBTN:
			//GetDlgItemTextA(hDlg, IDC_NICKNAMETEXT, nickname, SIZE_DAT);			
			GetDlgItemTextA(hDlg, IDC_NICKNAMETEXT, g_chatmsg.nickname, sizeof(nickname));
			return TRUE;
		/*case IDC_COLORRED:
			g_drawlinemsg.color = RGB(255, 0, 0);
			return TRUE;
...
DWORD WINAPI ReadThread(LPVOID arg)
{
	...
	while (1) {
		...
		if (comm_msg.type == TYPE_CHAT) {
			chat_msg = (CHAT_MSG *)&comm_msg;
			//printf("%s", chat_msg->nickname);
			char std[256];
			strcpy(std, chat_msg->nickname);

			DisplayText("[%s] %s\r\n", std,chat_msg->msg); //g_chatmsg.nickname
		}
...
DWORD WINAPI WriteThread(LPVOID arg)
{
	...
	while (1) {
		...
		char str[128];
		strcat(str, g_chatmsg.nickname);
		strcat(str, g_chatmsg.msg);
		//strcat(g_chatmsg.nickname ,g_chatmsg.msg);
		
		retval = send(g_sock, (char *)&g_chatmsg, SIZE_TOT, 0); //g_chatmsg
